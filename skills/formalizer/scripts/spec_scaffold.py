# -*- coding: utf-8 -*
"""spec_scaffold.py —— 正式模型 骨架生成器。支持单模型 12 项；选模决策 含 M1/M2/M3 时生成组合模型链。
用法: python spec_scaffold.py --model <名> [--data <csv> --config <cfg> --decision <选模决策.md> --out <path>]"""
...
def main():
    ...
    L = ["---", "type: model-spec", "stage: formalization", "owner: deepseek", "status: draft",
         "upstream:", '  - "[[选模决策]]"', "downstream:", '  - "[[验证报告]]"', "---", "",
         "# 正式模型.md —— 正式模型（formalizer 产出）", ""]
    m_lines = []
    if decision:
        try:
    ...
    out = arg("--out", "E:/26数模国赛/流程产物/模型规格.md")
