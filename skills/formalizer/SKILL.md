---
name: formalizer
description: 正式建模（严格形式化）。以人工 决策日志.md 为最终模型结构的唯一决策来源，把人工选定的模型体系（可能是组合模型 M1→M2→M3）严格数学化，输出 12 项 正式模型.md 与模型链。禁止重新选模型、禁止替换人工方案、禁止偷偷加新模型。当主方案已定（决策日志 存在）、需要写正式数学模型时使用。
---
...
## 输入（决策日志 最高优先级）
- `决策日志.md` —— **最终模型结构的唯一决策来源**
- 拆题报告.md、数据规则.md、方法候选.md、反方记录.md（辅助）
...
## 确定性内核
- `scripts/spec_scaffold.py --model <模型名> [--decision <决策日志.md>] ...`：单模型生成 12 项骨架；决策日志 含 M1/M2/M3 时生成组合模型链骨架；
