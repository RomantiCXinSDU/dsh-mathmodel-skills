...
def main():
    ...
    if target and target in df.columns:
        ...
        if not target_is_class(s):
            if s.min() >= 0 and s.max() <= 1 and s.nunique() > 2:
                add("[[数据规则#规则 8]]", "比例目标 [0,1]", "有界响应 + 方差非齐",
                ...
                add("[[数据规则#规则 8]]", "计数目标", "非负整数 + 方差=均值假设" + ("+ 零膨胀" if zr > 0.3 else ""),
                    m, "计数型目标", "OLS 对计数目标失真；零多时普通 Poisson 低估零概率")
            elif cfg.get("quantile_hint"):
                add("[[数据规则#规则 2]]", "达标线/分位关注", "条件分位数而非均值",
                    "分位数回归 / 贝叶斯分位数", "题目关注达标阈值或高分位",
                    "均值回归回答不了'90%对象何时达标'")
            else:
                add("[[数据规则#规则 10]]", "连续目标", "回归任务",
    ...
    if hi:
        ...
        if len(in01) >= 3:
            ...
            if 0.9 <= rsum.mean() <= 1.1:
                add("[[数据规则#规则 5]]", "成分数据(总和恒定)", "单纯形约束 + 成分内在依赖",
        ...
        if ratio > 0.05:
            add("[[数据规则#规则 4]]", "重尾/离群", "离群点拉偏估计",
                "稳健回归 Huber / RANSAC / Theil-Sen / 分位数回归", "IQR 离群占比>5%",
                "OLS 对离群敏感，系数被拉偏")
    if time_col:
        add("[[数据规则#规则 2]]", "时间序列", "时序相关 + 顺序不可乱",
    ...
    if cens or cfg.get("censored"):
        add("[[数据规则#规则 3]]", "删失/检出限", "截断分布",
            "Tobit / 删失回归", "测量有上下限",
            "把低于检出限当 0 → 偏差")
    if cfg.get("error_required"):
        add("[[数据规则#规则 3]]", "输入测量误差", "X 有误差 → 系数衰减偏倚",
            "Errors-in-Variables + 蒙特卡洛误差传播", "题目要求分析检测误差影响",
            "忽略 X 误差的 OLS 系数有偏")
    for key, rule_ref, structure, essence, m, cond, risk in [
        ("queue", "[[数据规则#规则 2]]", "排队/服务系统", "随机到达+服务时间", "排队论 M/M/1、M/M/c + Little 定律", "系统存在排队等待结构", "静态均值估计忽略等待时间分布"),
        ("inventory", "[[数据规则#规则 10]]", "库存/补货决策", "需求不确定下成本权衡", "报童模型 / EOQ / 多期库存", "订购量/补货决策", "按点估计进货 → 缺货或积压"),
        ("game", "[[数据规则#规则 18]]", "多方策略互动", "策略相互影响", "博弈论：纳什均衡 / 演化博弈", "多主体决策互相影响", "单方优化忽略对手反应"),
        ("mechanism", "[[数据规则#规则 10]]", "物理/传播机理", "机理方程优于纯数据", "ODE / 差分方程 / SIR 类", "存在明确机理过程", "纯数据模型外推不可靠"),
    ...
    out = sys.argv[sys.argv.index("--out")+1] if "--out" in sys.argv else "E:/26数模国赛/流程产物/方法候选.md"
