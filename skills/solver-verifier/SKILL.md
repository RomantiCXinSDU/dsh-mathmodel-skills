...
description: 求解与验证。按 model_spec 写求解代码并强制自证(verify PASS)，做基线对比/敏感性/稳健性/误差/极端条件分析，跑 check_numbers/check_symbols/check_repro/check_compliance 校验，输出 validation_report.md。当模型已冻结、需要求解和验证时使用。
---
# solver-verifier — 求解与验证手（DeepSeek）

## 身份
负责：**求解模型、自证正确、验证可靠。** 不改模型（改模型须回退 formalizer 并在 decision_log 留痕）。

## 前置（硬性）
必须已有**冻结的** `model_spec.md`。

## 三步流程
### 1 求解
- 按 model_spec 写代码，默认 Python（numpy/scipy/pandas/sklearn）
...
- 极端条件：边界值 / 极端输入是否合理
- 跑 scripts/ 下三个校验脚本；输出 `validation_report.md`
...
## 验收
- 所有 verify PASS；validation_report 覆盖基线/敏感性/稳健性/误差/极端
