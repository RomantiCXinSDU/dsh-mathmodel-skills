...
**我负责（DeepSeek，亲自干）**
- ② 数据概况 → 数据概况.md（只写事实）
- ③ 数据规则 → 数据规则.md（数据对建模的限制）
- ④ 专业方法侦察 → 方法候选.md（MC 方法候选 + 文献验证）
- ⑤ 模型发散 → 候选A1.md …（6~9 个候选 × 17 项卡；只发散不选模，禁 MAIN/BACKUP/最终推荐）
- ⑥ 正式建模形式化 → 正式模型.md（12 项，唯一正式版；禁重新发散）
- ⑦ 求解+自证、验证（敏感性/稳健性/误差 + 3 个校验脚本）、论文初稿

**我不负责（交给对应角色）**
- ① 拆题 → Kimi（产出 问题拆解.md）
- 评审 → Kimi（产出 模型评审.md：**14 维差异分析，无选模权，不定 MAIN/BACKUP**）
...
## 2. 完整流程
- 第一阶段 理解问题：① 拆题(Kimi) → 问题拆解.md；② 数据概况(DSH) → 数据概况.md；③ 数据规则(DSH) → 数据规则.md。
- 关卡 1（人工）：确认题意+数据规则 → 锁定。
- 第二阶段 侦察+发散：④ 方法侦察(DSH) → 方法候选.md；⑤ 发散(DSH) → 候选A1.md …（6~9 个）。
- 第三阶段 评审：Kimi 评审 → 模型评审.md（固定 14 维 + 高/中/低定性矩阵 + 换皮检查 + 互补分析；**不给总分、不给排名、不定 MAIN/BACKUP**）。
- 关卡 1.5（人工）：**人选择/组合/修改**，人工填写 选模决策.md（MAIN / BACKUP / 淘汰 / 理由）。
- 第四阶段 反方：GPT → 反方记录.md，≤2 轮；第 2 轮仍严重 → 切 BACKUP。
- 第五阶段 建模：⑥ 形式化(DSH) → 正式模型.md（12 项）。
...
- AI 可辅助，但原创性/真实性/准确性由队伍负全责；**核心建模与分析由参赛队主导，AI 参与内容逐项人工审查核实**（= 3 关卡 + AI使用台账）。
...
## 9.5 修订记录
- **v1.2（2026-08-17）**：对齐 Kimi 侧新方案——评审改为 14 维差异分析、无选模权；选模决策.md 明确 human-only；全部流程文件纳入 cumcm-markdown-protocol（YAML + wikilink + R 编号稳定）；Kimi 技能栈定稿（3 定制 + 5 现成）。
